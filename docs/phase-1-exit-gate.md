...
|---|---|---|
| 1 | Determinism gate 100/100, zero divergence | `dh-cli gate --runs 100`: `PHASE-1 DETERMINISM GATE: PASS (100 runs each)` — plain + timer sub-gates, every fingerprint identical (icount 2,000,000; state hash `7e09ac13…`; timer delivered at 1,234,567) |
...
| 5 | M3 accepts green | `timer_determinism` (100 runs × 10 fires, zero divergence, 95.7 s) ok; `if0_deferral` ok; `regression` (1e9 ×2, state-hash chains identical, 5.5 s) ok; `m1_acceptance` (full device surface, run-twice bit-identical incl. device snapshots) ok |
