...
| Linux RunWithFrameCapture (capture-neutrality C5, cancel, watchdog, live inject) | Operator-run acceptance — REQUIRED before shipping any frame/capture/hash-path change: the normative C5 neutrality of the real framebuffer-reading hook is only exercised here | `DH_M9_ALLOW_SKIP=0 cargo test -p dh-worker --test frame_capture_stream --release -- --ignored --nocapture --test-threads=1` |
| Linux play-path perf smoke (streamed-vs-per-frame fps ratio) | Operator-run acceptance | `DH_M9_ALLOW_SKIP=0 cargo test -p dh-worker --test play_perf_smoke --release -- --ignored --nocapture` |
