...
| Linux GS-7 inject-point replay | `DH_M9_ALLOW_SKIP=0 cargo test -p dh-worker --test gs7_inject_replay --release linux_gs7_inject_points_verify_replay_from_dhilog -- --ignored --nocapture` | ADDED gate, pending sibling fixture evidence: the test requires multiple post-READY Linux `InjectQuery` events, matching detchannel `PORT_INJECT` `PIO_ANSWER` records in the DHILOG, one workload-observed decision proof LogLine with prefix `gs7.inject_decisions=`, at least two distinct non-`Proceed` decisions, and VerifyReplay `Done` from the READY snapshot using only the DHILOG. |
...
| Linux M8 replay-commit (unfiltered `m7_fork_verify` run) | `… --test m7_fork_verify --release -- --ignored --nocapture --test-threads=1` | PASS: `M8 Linux replay-commit done: replay_commits=1000 unique_refs=1000 unique_hashes=1 replay_slots=3`; semantic-negative ref-mismatch test PASS (`2 passed`). |
...
Same run as the Phase 1 re-baseline section (plan
`epoch-023-rebaseline-and-intel-box-stack-redeploy`, code `8a97875`, dist
`workload-image-0.2.0`, artifact hashes in `docs/phase-1-exit-gate.md`).
Frame caps were re-derived from the measured 32.09M instr/frame
(`crates/dh-worker/tests/common/mod.rs`), the M9 corpus was regenerated
(`emu_version=refwork-emu 0.2.3`, epoch grid 50M, `meta_frame` replaces the
retired fixture-only pv-blk proof), and `dh-m9-ready-handoff` now refuses a
staged image whose hashes/epoch disagree with the corpus pins.

| Gate | Command | Evidence |
|---|---|---|
| Linux M4 snapshot/restore/fork transparency | `… --test m4_transparency --release linux -- --ignored --nocapture` | PASS: mid `icount=644240584`, hash `e59bdb8994cdf46cc182ce1ef5d03211e81fd29fa06a7e0332cdf8135589959c`; control/restored `icount=645240584`, hash `ae169fb5701a5b69b248835bf55cb8c3bc41297c0326e94c025ffb63471c8351`; restored-mid and restored hashes matched; `1 passed`. |
| Linux M5 frame scheduling | `… --test m5_frame_scheduling --release linux -- --ignored --nocapture` | PASS: `first_icount=715163953`, first frames `[(7748368, 1), (39835845, 2), (71923369, 3)]`, `restored_icount=779340130`, restored frames `[(32088347, 4), (64176177, 5)]`; NOP-game diagnostic frame 9,235,201; `2 passed`; identical across three runs today. |
| Linux M5 deterministic I/O loopback | `… --test m5_net_loopback --release linux -- --ignored --nocapture` | NOT GREEN (known gap): Run leg reaches BUDGET_REACHED (`io_frame_cost=7748368`, twice identical) but the test then requires the fixture-era `PVBLKIO1` meta proof, which the real refwork-harness never writes (no guest-driven post-READY pv-blk IO). Tracked in `bn` as part of `determinism-hypervisor-vitb` / cross-repo residue (history `jyo7`). |
| Linux M5 post-READY record/replay corpus | `… --test m5_record_replay --release linux_m5_record_replay_post_ready_corpus_reverifies -- --ignored --nocapture` | PASS ×3 today against the regenerated `expected.txt`: `machine_config_hash=3f64c294ed51aee05a4339a2730ec4d06eacc1314a71931568b06027218ca768`, `ready_snapshot_ref=b2e4d21944975c4172d17b34e08667563697f2b17710a7e1a308f06ed3db6853`, `end_snapshot_ref=175b2717b72b1c55b5812f9aa8469788c84217603b0051e19bb8ce74ef7cbb19`, `dhilog_blake3=f11ec2ac4122c39c96243a5eb02b02927ec4e176f256738520c483a46eac7aa8`, `epoch_hashes_verified=2`, `end_icount=136099546`, `end_state_hash=ec4a34a8df509b6631fc65a813d19ccd5eaff7ea3331f26888d739e2b421f4a8`, `frame_counter=5`, `meta_frame=5`. |
| Linux worker API support | `… --test linux_worker_api --release -- --ignored --nocapture` | PARTIAL: `pvblk_dev_vdb_phase_a_fixture_ready_regions` PASS; `pvblk_dev_vdb` FAIL in its VerifyReplay-to-READY leg with `OVERSHOOT: counted 643241128 past target 643240584` (landing-engine skid at the READY transition on this image; `determinism-hypervisor-vitb`). CreateVm/Run/Snapshot/Restore legs passed before the replay leg. |
| Linux GS-7 inject-point replay | `… --test gs7_inject_replay --release …` | NOT GREEN, unchanged from 2026-06: the fixture emits 0 `InjectQuery` events (needs guest-side inject-point traffic). |
| Linux RunWithFrameCapture (C5 neutrality, cancel, watchdog, live inject) | `… --test frame_capture_stream --release -- --ignored --nocapture --test-threads=1` | PASS: `4 passed` in 70.7 s (required because the cap constants changed). |
| Linux play-path perf smoke | `… --test play_perf_smoke --release -- --ignored --nocapture` | PASS: 31,953,178 instr/frame, streamed/per-frame fps ratio 1.93×, `1 passed`; identical across three runs today. |
