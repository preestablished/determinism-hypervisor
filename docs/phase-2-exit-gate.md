...
| Linux GS-7 inject-point replay | `DH_M9_ALLOW_SKIP=0 cargo test -p dh-worker --test gs7_inject_replay --release linux_gs7_inject_points_verify_replay_from_dhilog -- --ignored --nocapture` | ADDED gate, pending sibling fixture evidence: the test requires multiple Linux `InjectQuery` events, matching detchannel `PORT_INJECT` `PIO_ANSWER` records in the DHILOG, at least two distinct non-`Proceed` decisions, and VerifyReplay `Done` from the READY snapshot using only the DHILOG. |
