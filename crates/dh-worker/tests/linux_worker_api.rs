...
fn assert_ready_ordering(events: &[proto::GuestEvent], ready_icount: u64) -> TestResult<()> {
    ...
    let region = event_pos(events, detguest_wire::record::EventKind::RegionRegister)
        .ok_or_else(|| "guest events did not contain RegionRegister before Ready".to_string())?;
    let ready = event_pos(events, detguest_wire::record::EventKind::Ready)
        .ok_or_else(|| "guest events did not contain Ready".to_string())?;
    if !(hello < ready && region < ready) {
        return Err(format!(
            "guest event order invalid: hello={hello}, region={region}, ready={ready}"
        ));
    }
...
fn snapshot_tags(
    ...
    let dhsnap = Container::parse(&manifest.device_blob.bytes)
...
fn pvblk_dev_vdb() {
    ...
    rt.block_on(async {
        ...
        assert_eq!(
            ...
            "Ready must be observed through detchannel next_sdk_event"
        );
        ...
        assert!(
            ready.region_count >= 3,
            "fixture should publish at least wram, framebuffer, and meta regions: {ready:?}"
        );
        assert_eq!(
        ...
        assert_ready_ordering(&events, run.icount).expect("Ready ordering and /dev/vdb evidence");
