...
/// (emulator epoch refwork-emu 0.2.3, built_from ce754be; initramfs blake3
/// 941fff70…): max ~32.09M instr/frame (fresh f1=7.75M, f2/f3=32.09M;
/// post-restore f4/f5=32.09M; NOP-game diagnostic frame 9.24M), identical
/// across two consecutive runs (2026-07-07 on 0.1.0: 27.7M). Cap =
/// 32.1M/frame × 3 frames (FIRST_FRAMES) × ~1.8 margin ≈ 175M — a safety
/// net for runaway boots, not a perf assertion; the real-path normal stop
/// is BUDGET_REACHED. Retune by re-running:
/// cargo test -p dh-worker --release --test m5_frame_scheduling -- \
///   --ignored linux_m5 --nocapture
const FRAME_HARD_CAP: u64 = 175_000_000;
